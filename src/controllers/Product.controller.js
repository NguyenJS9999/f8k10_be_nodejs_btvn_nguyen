...
const ProductController = {
	...
	createProduct: async (req, res) => {
		...
		try {
            ...
            const exitCategory = await CategoryTable.findById(req.body.categoryId);
			if (!exitCategory) {
			...
			const existingProduct = await ProductsTable.findOne({
				product_code
			});
			if (existingProduct) {
            ...
            exitCategory.products.push(newProduct._id);
            await exitCategory.save();
	...
	createProducts: async (req, res) => {
		try {
			...
			const inputCodes = productsWithUppercaseCode.map(
			...
			const existingProducts = await ProductsTable.find({
				product_code: { $in: inputCodes }
			});
			const existingCodes = existingProducts.map(
				product => product.product_code
			);
			...
			const newProducts = await ProductsTable.insertMany(
				productsWithUppercaseCode
			);
	...
	updateProduct: async (req, res) => {
		try {
			...
			if (
				product_code &&
				product_code.toUpperCase() !== findProduct.product_code
			) {
				const upperCaseCode = product_code.toUpperCase();
				const existingProduct = await ProductsTable.findOne({
	...
	deleteProducts: async (req, res) => {
		...
		try {
			...
			if (!ids || !Array.isArray(ids) || ids.length === 0) {
				return res.status(400).json({
					message:
						'Invalid or missing ids. Please provide an array of ids to delete.'
				});
			}
			...
			const deleteResult = await ProductsTable.deleteMany({
				_id: { $in: validIds }
			});
			...
			return res.status(200).json({
				...
				notDeletedIds:
					validIds.length > deletedCount
						? validIds.filter(
								async id =>
									!(await ProductsTable.exists({ _id: id }))
						  )
						: []
			});
		} catch (error) {
